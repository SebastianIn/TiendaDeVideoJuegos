...
export class JuegoComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.juegoService
      ...
      .subscribe(
        (res: HttpResponse<IJuego[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IJuego): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInJuegos(): void {
    this.eventSubscriber = this.eventManager.subscribe('juegoListModification', () => this.loadPage());
  }
